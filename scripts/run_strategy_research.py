...
def render_news(meta: dict, signals: pd.DataFrame, leaderboard: pd.DataFrame, peak_timing: pd.DataFrame) -> str:
    lines = [
        ...
        "",
        "Hold research: **5, 10, 15, 30, 45, 60, 90, 120, 180, 240 minutes + EOD**, with exact time-to-peak.",
        "",
    ]
    ...
    if not leaderboard.empty:
        cols = [c for c in [
            "strategy_id", "variant_id", "direction", "rule_id", "max_hold_minutes", "validation_n",
    ...
    if not peak_timing.empty:
        cols = [c for c in [
            "strategy_id", "variant_id", "direction", "split", "market_cap_bucket", "n_signals",
